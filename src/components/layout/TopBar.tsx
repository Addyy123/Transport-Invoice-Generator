...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  // Bug #20: Show actual user name/email instead of always "Admin"
  const getDisplayName = () => {
    if (isGuest) return 'Guest Tester';
    return (
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-card px-4 md:px-6 relative z-30 transition-colors duration-200">
      ...
      <div className="flex flex-1 items-center justify-end gap-3 md:gap-4 relative">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-card text-card-foreground ring-1 ring-black ring-opacity-5 border border-border focus:outline-none z-50">
        ...
        <div className="relative" ref={profileRef}>
          ...
          {showProfile && (
            <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-card text-card-foreground ring-1 ring-black ring-opacity-5 border border-border focus:outline-none z-50 py-1">
              <div className="px-4 py-2 border-b border-border mb-1">
                <p className="text-sm font-medium">{isGuest ? 'Local Test Mode' : 'Administrator'}</p>
                <p className="text-xs text-muted-foreground truncate">
                  {isGuest ? 'guest@localhost' : (user?.email || 'admin@local')}
                </p>
              </div>
              <button
                onClick={() => { setShowProfile(false); navigate('/profile'); }}
                className="w-full text-left px-4 py-2 text-sm text-card-foreground hover:bg-slate-50 flex items-center gap-2 transition-colors"
              >
